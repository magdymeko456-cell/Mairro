...
echo "⏳ 1. تحديث منطق اللغة في home_screen.dart لحفظ آخر لغة والاعتماد على لغة الجهاز..."

# استخدام sed أو إعادة كتابة الدالة لتكون مأمنة ومطابقة لطلبك
cat << 'INNER_EOF' > patch_home.dart
...
mv patch_home.dart "$HOME_FILE"

echo "⏳ 2. رفع الكود الجديد والمستقر لجيت هب..."
git add "$HOME_FILE"
git commit -m "feat: persistent last used language and fallback to device system language"
git push origin main

echo "🚀 تم التحديث والرفع بنجاح يا تامر! البناء الجديد هيكون مأمن تماماً وبيحفظ الذاكرة."
